  ...
  return (
    <nav className="flex items-center justify-between px-6 py-4 fixed top-0 left-0 right-0 z-50 bg-[#11100F]">
      <div className="flex items-center space-x-2">
        ...
        <span className="text-xl font-medium tracking-wide text-[#E8E6E3] font-serif">
          AI
        </span>
      </div>

      <div className="hidden lg:flex items-center space-x-8 text-sm font-medium text-[#A6A29C]">
        {["Meet AI", "Platform", "Solutions", "Pricing", "Resources"].map(
          (item) => (
            <button
              key={item}
              className="flex items-center space-x-1 hover:text-[#E8E6E3] transition-colors"
            >
              <span>{item}</span>
